...
namespace NetService
{
    ...
    internal sealed class Session : IDisposable, IMessageContext
    {
        ...
        public void Open(SocketAsyncEventArgs receiveEventArgs, SocketAsyncEventArgs sendEventArgs)
        {
            ...
            //TODO:최흥배. 지정 시간안에 CompletedUpdateTimer 처리를 완료하지 못한 경우에는 기다렸다가 호출하는지 혹은 다른 스레드에서 시간에 맞게 호출하는지 확인 필요.
            // 타이머 셋팅
        ...
        public void PostSend(byte[] buffer, int offset, int length, bool directly)
        {
            ...
            byte[] copy = Service._pooledBufferManager.Take(length);
        ...
        private void UpdateFlushSending()
        {
            ...
            //TODO:최흥배. 멀티스레드에서 send를 하면 _sendingArraySegmentList가 경쟁 상태에 빠진다. 락 걸어야 하거나 하나의 스레드에서만 호출해야 한다.
            _sendingArraySegmentList.Clear();
        ...
        private void UpdateZombie()
        {
            ...
            if (NetService.Util.Tick.Gap(_lastActivateTime, Environment.TickCount) > Service.Config.SessionReceiveTimeout)
            {
